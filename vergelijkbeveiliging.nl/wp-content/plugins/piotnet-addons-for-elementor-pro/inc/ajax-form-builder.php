			...
			foreach ($fields as $field) {
				...
				if (!empty($repeater_id) && !empty($repeater_label) && $repeater_remove_this_field == false) {
					if (strpos($message, $repeater_label) !== false) {
						$message_all_fields .= $field['label'] . ': ' . $field_value . '<br />';
					} else {
			...
			foreach ($fields as $field) {
				...
				if (!empty($repeater_id) && !empty($repeater_label) && $repeater_remove_this_field == false) {
					if (strpos($repeater_content, $repeater_label) !== false) {
						if (strpos($field['name'], 'pafe-end-repeater') === false) {
							$string_add = $field['label'] . ': ' . $field_value . '<br />';
						}
	...
	function pafe_get_field_value($field_name,$fields, $payment_status = 'succeeded', $payment_id = '', $succeeded = 'succeeded', $pending = 'pending', $failed = 'failed' ) {
		...
		if (strpos($field_name, '[repeater id') !== false) { // ] [ [ fix alert
			...
			foreach ($fields as $field) {
				...
				if (!empty($repeater_id) && !empty($repeater_label) && $repeater_remove_this_field == false) {
					if (strpos($repeater_content, $repeater_label) !== false) {
						$string_add = $field['label'] . ': ' . $field_value . '\n';
						pafe_merge_string($repeater_content,$string_add);
					} else {
		...
		global $wpdb;
			if ( !empty($_POST['post_id']) && !empty($_POST['form_id']) && !empty($_POST['fields']) ) {
				...
				if ($recaptcha_check == 1) {
					...
					// 	$activecampaign_list = $form['settings']['activecampaign_list'];

					// 	$activecampaign_email = pafe_get_field_value( $form['settings']['activecampaign_email_field_shortcode'], $fields );

					// 	$activecampaign_field_mapping_list = $form['settings']['activecampaign_field_mapping_list'];

					// 	if ( !empty($activecampaign_email) && !empty($activecampaign_api_key) && !empty($activecampaign_list) ) {

					// 		$activecampaign_url = 'https://api.activecampaign.com/api/v2/groups/' . $activecampaign_list . '/subscribers';

					// 		$activecampaign_body = array(
					// 			'email' => $activecampaign_email,
					// 		);

					// 		if (!empty($activecampaign_field_mapping_list)) {
					// 			$activecampaign_fields = array();
					// 			foreach ($activecampaign_field_mapping_list as $item) {
					...
					// 			$activecampaign_body['fields'] = $activecampaign_fields;
					// 		}

					// 		$activecampaign_request_data = [
					// 			'headers' => array(
					// 				'X-activecampaign-ApiKey' => $activecampaign_api_key,
					// 				'Content-Type' => 'application/json',
					// 		    ),
					// 		    'body' => json_encode( $activecampaign_body ),
					// 		];

					// 		$activecampaign_request = wp_remote_post( $activecampaign_url, $activecampaign_request_data );
