		...
		if ( $(this).data('pafe-form-builder-stripe-submit') == undefined ) {
			...
			$fields.each(function(){
				if ( $(this).data('pafe-form-builder-stripe') == undefined && $(this).data('pafe-form-builder-html') == undefined ) {
					...
					if ( !$(this)[0].checkValidity() && $(this).closest('.elementor-widget').css('display') != 'none' && $(this).closest('[data-pafe-form-builder-conditional-logic]').css('display') != 'none' && $(this).data('pafe-form-builder-honeypot') == undefined &&  $(this).closest('[data-pafe-signature]').length == 0 || checked == 0 && $checkboxRequired.length > 0) {
...
jQuery(document).ready(function($) {
	            	...
	            	for (var jx = 0; jx<fieldNameArray.length; jx++) {
	                    ...
	                    if($fieldSelectorExist.length > 0) {  
		                    ...
		                    if (fieldName.indexOf('[]') !== -1) {
		                        ...
		                        if ($repeater_field.length > 0) {
		                        	if($fieldSelectorMultiple.length > 1 || $fieldSelectorMultiple.length == 1 && $fieldSelector.closest('[data-pafe-form-builder-repeater-id]')) {
	...
	$(document).on('keyup change','[data-pafe-form-builder-form-id]', $.debounce( 30, function(){
		var fieldName = $(this).attr('name').replace('[]','').replace('form_fields[','').replace(']','');
