...
(function ($) {
	...
	var WidgetPafeFormBuilderHandlerSignature = function ($scope, $) {
		...
		$.each($elements, function (i, $element) {
			...
			var $pafeSingature = $($element).closest('[data-pafe-signature]'),
				$clearButton = $pafeSingature.find('[data-pafe-signature-clear]'),
			...
			$exportButton.click(function(){
				if (signaturePad.isEmpty()) {
					$pafeSingature.find('.elementor-field').val('');
				} else {
					var url = signaturePad.toDataURL();
					$pafeSingature.find('.elementor-field').val(url);
				}
			});
